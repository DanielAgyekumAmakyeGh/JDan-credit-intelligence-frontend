...
# ============================================================
# PAGE 6: AFFORDABILITY TOOL WITH ML PREDICTION
# ============================================================
elif page == "Affordability Tool":
    st.header("Affordability Calculator with ML Prediction")
    ...
    if st.button("Calculate Affordability & Risk", type="primary"):
        ...
        if result:
            ...
            st.metric("Recommended Loan (12 months)", f"GHS {dti_data.get('recommended_loan', 0):,.0f}")
            
            decision = dti_data.get('decision', '')
            if decision == "APPROVE":
                st.success(f"DTI Decision: {decision}")
            elif decision == "LIMITED":
                st.warning(f"DTI Decision: {decision}")
            else:
                st.error(f"DTI Decision: {decision}")
            ...
            if decision == "APPROVE" and ml_data.get('decision') == "APPROVE":
                st.success("APPROVE - Both DTI and ML models suggest approval")
            elif decision == "DECLINE" or ml_data.get('decision') == "DECLINE":
                st.error("DECLINE - Either DTI or ML model suggests decline")
            else:
                st.warning("REVIEW - Mixed signals from DTI and ML models")
...
# ============================================================
    ...
    # Get lender performance for alerts
    lenders = api_get("/analytics/lender-performance")
    if lenders:
        high_risk = [l for l in lenders if l.get('default_rate', 0) > 20]
        if high_risk:
    ...
    purposes = api_get("/analytics/loan-purpose")
    if purposes:
        high_risk_purposes = [p for p in purposes if p.get('default_rate', 0) > 25]
        if high_risk_purposes:
            st.warning(f"⚠️ WARNING: {len(high_risk_purposes)} loan purpose(s) have default rates above 25%")
    ...
    if npl_data and len(npl_data) > 0:
        current_npl = npl_data[-1].get('npl_ratio', 0)
        if current_npl > 15:
